...
OUTPUT_CSV = "E:\\Synthetic-Email-Generation-tool-main\\internal_coordination\\internal_coordination.csv"
DEBUG_DIR = "E:\\Synthetic-Email-Generation-tool-main\\internal_coordination\\debug"
LOG_FILE = "E:\\Synthetic-Email-Generation-tool-main\\internal_coordination\\internal_coordination_history.log"  # <--- NEW LOG FILE
NUM_EMAILS = 700       
DELAY_SECONDS = 0.1      
MAX_RETRIES = 3          
...
BASE_USER_PROMPT = """Generate a realistic internal coordination email sent BETWEEN HR team members within the same company.
This email is used for internal hiring workflow coordination and is NOT sent to the candidate.

CONTEXT:
- The role being discussed is {role}
- The candidate is described as {adjective}

REQUIREMENTS:
- Invent realistic fictional names for the sender and internal recipient
- The sender and recipient names MUST be different in every email
- Do NOT reuse the same internal names across emails
- The email must reference internal hiring coordination activities
- Mention updating records, tracking status, or aligning next steps
- Do NOT directly address the candidate
- Do NOT include candidate contact details
- Do NOT confirm final hiring decisions
- Professional, neutral, internal corporate tone
- Short, realistic internal business email
- No double quotes anywhere
- Professional internal closing
...
SUBJECT: Candidate Status Update | Marketing Manager Position

BODY:
Hi Ankit,

I wanted to update you regarding the Marketing Manager profile we discussed earlier.

The client feedback has been logged in the system, and the candidate record has been updated accordingly. Please ensure that the next steps are aligned with the current hiring timeline and reflected in the tracking dashboard.

Let me know if any additional coordination is required.

Best regards,
Sonal

TONE: Professional, neutral, and internal

INTENT: Coordinate internal hiring workflow and update team on candidate status
